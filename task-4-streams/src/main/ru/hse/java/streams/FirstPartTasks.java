...
public final class FirstPartTasks {
    ...
    public static List<String> allTracksSorted(Stream<Album> albums) {
        return albums.flatMap(tr -> tr.getTracks().stream()).
                map(Track::getName).
                sorted().
    ...
    public static List<Album> sortedFavorites(Stream<Album> albums) {
        return albums.
                filter(al -> al.getTracks().stream().anyMatch(tr -> tr.getRating() > 95)).
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        ...
        return albums.
                filter(al -> !hashSet.add(al)).
                count();
        // throw new UnsupportedOperationException();

        //return albums.count() - albums.distinct().count();
        /*
        return albums.collect(
                Collectors.groupingBy(
                        Album::getArtist,
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.min(
                Comparator.comparing(al -> al.getTracks().stream().map(Track::getRating).
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.
                sorted(Comparator.comparing((Album al) -> al.getTracks().stream().mapToInt(Track::getRating).average().orElse(0)).reversed()).
                collect(Collectors.toList());
    }
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream.reduce(1, (acc, x) -> (acc * x) % modulo);
    }
